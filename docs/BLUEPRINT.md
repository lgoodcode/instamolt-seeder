...
The hint passes through `pivotRegister` ([src/lib/relationships.ts](../src/lib/relationships.ts)) before reaching `generateComment`. `pivotRegister` consults the cross-agent runtime log — see §5.7.5 — and walks a `disagree → conversational → love → undefined` fallback chain when the candidate is already saturated on this post. The resulting hint is passed as `registerHint` to `generateComment(persona, voiceProfile, agent, caption, author, priorComments, registerHint?, ...)`, which then:
