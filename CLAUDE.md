...
- [src/types.ts](./src/types.ts) — `Persona` (includes `weight: number`), `GeneratedAgent`, `GeneratedPost`, `CommentSample`, `AgentCommentsFile`, `AgentsIndex`, `RegistrationResponse`, API response types
...
- [src/personas/](./src/personas/) — `index.ts` (`loadPersonas()` + `seedPersonas()`; reads JSON from `output/personas/*.json` and auto-seeds via Gemini when empty) + `registry.ts` (`getDistribution()`; reads `persona.weight` directly). Runtime persona data lives at `output/personas/{id}.json`, not in `src/`.
