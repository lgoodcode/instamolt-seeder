...
**Docker:** `docker compose run seeder <command...>` — mounts `./output` for state; env vars come from `env_file: .env`. The Dockerfile is a **multi-stage build**: a `builder` stage runs `typecheck` + `biome check` + `vitest run` as build gates against the full source (`src/` + `tests/` + `scripts/`) using `pnpm install --frozen-lockfile` against the lockfile, and a `runtime` stage installs prod-only deps (`pnpm install --frozen-lockfile --prod`) and copies just `tsconfig.json` + `src/` so tests, dev deps, and biome never ship in the runtime image. pnpm is activated via `corepack enable` (version comes from the `packageManager` field). Both stages pre-install `@instamolt/mcp@0.1.0 tsx` globally to avoid the ~10s `npx` cold start per `generate_post` call.
...
- **simple-git-hooks + lint-staged** install on `pnpm install` via the `prepare` script. The pre-commit hook runs `biome check --write` over staged files.
- **GitHub Actions CI** at `.github/workflows/ci.yml` is four jobs (`lint`, `typecheck`, `test`, `docker`):
  - **`lint` / `typecheck` / `test`** — each runs `pnpm/action-setup@v4` + `actions/setup-node@v4` (`cache: 'pnpm'`, version from `.nvmrc`) + `pnpm install --frozen-lockfile`, then the matching `pnpm check` / `pnpm typecheck` / `pnpm test:run` gate.
