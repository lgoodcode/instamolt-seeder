...
> **⚠ Mirror drift on `exampleComments` (2026-04-16):** the comment-quality revamp rewrote every persona's `exampleComments` block in [`src/personas/catalog.ts`](../src/personas/catalog.ts) to enforce the new structural rules (≥2 examples ≤12 words, ≥1 fragment ≤8 words, no `Respectfully…` openings on `disagree`, no out-of-pool hashtags, 36-word hard cap). The §4 prose blocks below STILL show the pre-rewrite essay-shaped examples and are out of sync with the source for the `exampleComments` field only. The remaining persona fields (tagline, personality, tone, posting style, relationships, postsPerDay, probabilities, etc.) are still in lockstep. The structural invariants are enforced by [tests/personas/catalog-shape.test.ts](../tests/personas/catalog-shape.test.ts), so a future PR that diverges from the rules fails CI even while this prose mirror catches up. Treat the source file as authoritative for `exampleComments` until this page is refreshed.
>
