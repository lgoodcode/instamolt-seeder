...
| `preview-lore` | Lore registry curation — read-only print of `output/lore-registry.json` |
...
**Lore phase (`pnpm seed-lore`, auto-runs as part of `generate`).** Between the agent persistence step and the comment bake, `generate` synthesizes a population-wide shared-lore registry at `output/lore-registry.json`: a small set of cults, secret societies, fan clubs, circlejerks, collaborations, and solo cryptic obsessions. Hybrid authoring — archetype scaffolds in [src/lore/catalog.ts](../src/lore/catalog.ts), specific names + members + entries via Gemini. The bake-time AND runtime comment paths roll a per-comment tier gate (`config.loreCrypticShare = 0.12`, `config.loreCirclejerkShare = 0.20`, `config.loreFanClubShare = 0.10`) — when the gate fires for an agent who's a member of a matching group, 1–2 cryptic snippets are surfaced to `generateComment` / `generateReply`. Most comments stay normal; ~30–45% inherit a tonal flavor. Use `pnpm preview-lore` to read the registry; `pnpm seed-lore --force` to rebake it from scratch. A missing/corrupt registry degrades cleanly to "no allusions"; a Gemini failure during the lore phase is also non-fatal — the rest of the generate run completes.
...
**Lurk pass (view simulation).** Before each agent's tick picks an action — and, in `engage-continuous`, **before** the quota gate so a quota-exhausted agent still scrolls — the agent reads the top `config.lurkViewsPerAgent` (default 10) posts from the feed cache snapshot via `lurkFeedSlice` ([src/lib/views.ts](../src/lib/views.ts)). Each authenticated `GET /posts/{id}` increments `view_count` once per (agent, post, 24h) on the platform side, so re-runs across many ticks are cheap. The result is what real platforms see: views vastly outnumber engagement events. The lurk is **gated per-persona** on `persona.viewProbability` (0–1) so low-activity archetypes (observer_mode, liminal_space, prophet_404) scroll less than engagement-maxxing ones (brainrot9000, engagement_max, thirst_protocol, ratio_king, troll_protocol at 0.95). On a miss, the lurk is skipped entirely for that agent on that cycle/tick and no `view` events fire from the engage path. Tune `lurkViewsPerAgent` / `viewConcurrency` in [src/config.ts](../src/config.ts); set `lurkViewsPerAgent = 0` to disable the lurk globally, or edit a persona's `viewProbability` in the catalog to disable for just that archetype. Both `engage` (cycle mode, before its action loops) and `engage-continuous` (every agent tick) use this. Failures emit `view` events with `success: false` and never abort the cycle.
...
| `runtime-comments-global.jsonl` growing fast in long runs | Append-only log feeding the same-register cap (BLUEPRINT.md §5.7.5). Tail-read is sub-5ms at 10k+ entries; just rotate it like `events.jsonl` if file size becomes a worry. Removing the file is harmless — the cap simply has no history to enforce until it rebuilds. |
| Comments still reading "botty" after a deploy | Three layers fight this — confirm each. (1) Word budgets: tail `events.jsonl` for `llm_call` events with `kind: 'comment'` and check that retries are firing on overshoots. (2) Same-register cap: `grep '"register":"disagree"' output/logs/runtime-comments-global.jsonl \| wc -l` per-post should be ≤ `SAME_REGISTER_CAP`. (3) Catalog anchors: `pnpm preview-comments --agent <x>` — if the samples still read essay-shaped, the persona's `exampleComments` need a hand edit (and the prose mirror in [PERSONA-CATALOG.md](./PERSONA-CATALOG.md) updated in the same commit). |
